...
// Go's `tracing.Hooks` is a struct of optional EVM callback function pointers.
// Rather than binding to revm's full `Inspector` trait (which is generic over the
// EVM wiring/database and would force those generics to propagate into every storage
// type that holds a `TracingInfo`), we define a minimal trait with only the two
// callbacks that ArbOS actually uses: storage get and storage set notifications.
// Implementors can wrap a revm `Inspector` or any other tracer behind this interface.
pub trait ArbitrumTracer {
    fn capture_storage_get(&self, key: B256, depth: u32, before_evm: bool);
    fn capture_storage_set(&self, key: B256, value: B256, depth: u32, before_evm: bool);
}

// Go's `*tracing.Hooks` is nullable, so `tracer` is `Option<Box<dyn ArbitrumTracer>>`.
// `Contract` is omitted for now — it is only needed for DuringEVM opcode tracing,
// which is not yet implemented.
pub struct TracingInfo {
    pub tracer: Option<Box<dyn ArbitrumTracer>>,
